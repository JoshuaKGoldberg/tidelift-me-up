...
- `--status` _(default: `'all'`)_: If provided, a filter on package lifting status: `'all'`, `"available"`, `"lifted"`, or `"needs-subscribers"`
  - `"needs-subscribers"` lists packages Tidelift doesn't yet estimate income for because they don't have enough subscribers; these are excluded from `'all'`
